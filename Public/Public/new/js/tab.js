...
$(".shoptype .radiobox li").on('click',function(){
	...
	if(!$(this).hasClass('active')){
		if($(this).hasClass('isshop')){
			...
			if($(".relashow .relation").hasClass('active')&&$(".relationbox .relation").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box>input').addClass('required')
			}
		}else{
			$(".licence").fadeOut()
			...
			if($(".relashow .relation").hasClass('active')&&$(".relationbox .relation").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box').show().end().find('.pic_box>input').addClass('required')
			}

		}
		if($(".accounttype .relation ").hasClass('active')){
			$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box>input').removeClass('required')
		}
...
$(".relationbox li").on('click',function(){
	if(!$(this).hasClass('active')){
		if($(this).hasClass('relation')){
			...
			if(!$(".shoptype .radiobox .isshop").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box').show()
			}
		}else{
			...
			if(!$(".shoptype .radiobox .isshop").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box').hide()
			}
		}
		$(this).addClass('active').find('input').attr('checked','true').end().siblings().removeClass('active').find('input').removeAttr('checked')
	}
...
$(".relation-info .radiobox li,.leadsex li,.accounttype li").on('click',function(){
	...
	$('.accounttype li').on('click', function () {
		if($(this).hasClass('relation')){
			...
			if($(".shoptype .radiobox .isshop").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box>input').removeClass('required').end().find('span>i').hide()
			}else {
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box').hide().end().find('.pic_box>input').removeClass('required')
			}
		}else {
			...
			if(!$(".shoptype .radiobox .isshop").hasClass('active')&&$(".relationbox .relation").hasClass('active')){
				$('.uploadimg ul').eq(2).find('li').eq(0).find('.pic_box').show().end().find('.pic_box>input').addClass('required')
			}
		}
	});
