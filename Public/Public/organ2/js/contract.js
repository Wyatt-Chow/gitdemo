...
function fee(id){
	for(var p in json_fee){
		if(id==json_fee[p]['pmf_id']){
			var str='';
			var n='';
				str+='<p><input type="text" id="fee" name="fee" value="'+json_fee[p]['pmf_max_fee']+'" class="txt w2 required" style="width:40px"> % ';
				if(json_fee[p]['pmf_max_fee']!=json_fee[p]['pmf_min_fee']){
					str+='<span>('+json_fee[p]['pmf_min_fee']+'% - '+json_fee[p]['pmf_max_fee']+'%)</span>';
				}
				if(parseInt(json_fee[p]['pmf_limit'])>0){
					str+='&nbsp;<input class="txt w2" style="width:40px" type="text" id="pmf_limit" name="pmf_limit" value="'+(json_fee[p]['pmf_limit']/100)+'"> '+caps;
				}else{
