$(function(){
	...
	$(".close_menu").click(function(){
		...
		if(close_menu=='show'){
			...
			$("#pane").show();
			$(".content").css({"padding-left":"160px"});
			$(".close_menu").css({"left":"160px","background-position":"0px 0px"});
	...
	$(".menu h3").click(function(){
		if($(this).next("ul").is(":hidden")){
			$(this).siblings('h3').removeClass('cur').end().siblings('ul').hide().end().addClass("cur").next("ul").show();
	...
	$("a.ht_a").click(function(){
		if($(this).next(".show_sub_menu").is(":hidden")){
			$(this).addClass("cur").next(".show_sub_menu").show().end().parent().siblings().find('.ht_a').removeClass("cur").next(".show_sub_menu").hide();
		}else{
			$(this).removeClass("cur").next(".show_sub_menu").hide();
		};
	});
...
function up_pic_obj(obj,pic,F_SERVER){
	...
	if(F_SERVER==undefined || F_SERVER==''){
		...
		if(ispic){
			$('#'+obj+'_pic').html('<a href="'+F_SERVER+pic+'" data-lightbox="image-1" data-title="pic"><img style="float:left" height="80" width="120" src="'+F_SERVER+pic+'" onerror="this.src=\'/upfile/files?storage=1&f='+pic+'\'"></a>');
		}else{
...
function up_pic_obj3(obj,pic,F_SERVER){
	...
	if(F_SERVER==undefined || F_SERVER==''){
		if(ispic){
			...
			$('#'+obj+'_pic').html('<a href="/upfile3/files?f='+pic+'" data-lightbox="image-1" data-title="pic"><img style="float:left" height="80" width="120" src="/upfile3/files?f='+pic+'"></a>');
		}else{
			$('#'+obj+'_pic').html('<a href="/upfile3/files?f='+pic+'" style="display:block" title="'+file_ext+'" target="_blank"><img style="float:left" height="80" width="120" src="/Public/organ2/images/file.jpg"></a>')
		}
	}else{
		if(ispic){
			console.log(F_SERVER+pic);
			$('#'+obj+'_pic').html('<a href="'+F_SERVER+pic+'" data-lightbox="image-1" data-title="pic"><img style="float:left" height="80" width="120" src="'+F_SERVER+pic+'" onerror="this.src=\'/upfile3/files?storage=1&f='+pic+'\'"></a>');
		}else{
			console.log(F_SERVER+pic);
			$('#'+obj+'_pic').html('<a href="'+F_SERVER+pic+'" style="display:block" title="'+file_ext+'" target="_blank"><img style="float:left" height="80" width="120" src="/Public/organ2/images/file.jpg"></a>')
		}
	...
	$("input[type='password']").each(function(){
		...
		if(password_str!=''){
			if($(this).attr("add")=="add"){
				var Regx = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,100}$/;//数字和字母的组合
		...
		success: function (data) {
			...
			if(data['status']==1){
				...
				if(typeof(json[0])=="undefined"){
					...
					if(typeof(json[0]['shop_no'])=='string'&&typeof(json[0]['mct_no'])=='string'){
						errorcode=0;
						newdata='&shop_no='+json[0]['shop_no']+'&mct_no='+json[0]['mct_no'];
						console.log(newdata)
					}
				}
			};
...
function checkSearch(form){
	if($("input[name='sdate']").length>0 && $("input[name='edate']").length>0 && $("input[name='sdate']").val()!='' && $("input[name='edate']").val()!='') {
        var formatDate1 = new Date($("input[name='sdate']").val());
        var formatDate2 = new Date($("input[name='edate']").val());
        if (formatDate1 > formatDate2) {
            layer.tips(input_tips('开始日期不能大于结束日期'),$("input[name='sdate']"),{tipsMore: true,tips: [2, '#000000']});
            return false;
        }
    }
}
