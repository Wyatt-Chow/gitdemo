      ...
      <el-row style="margin-bottom:20px">
        <el-col :span="24">
          ...
          <el-date-picker
            ...
            <label style="padding-left: 30px">话费渠道:</label>
            <el-select style="width:20%" size="mini" v-model="params.channel">
    ...
    <el-container style="padding: 10px 10px;border: 1px solid #ebebeb; border-radius: 4px;">
      <el-header style="height: 600px;overflow:auto;">
        <el-table ref="multipleTable" :data="tableData" stripe border tooltip-effect="dark" style="width: 100%">
          <el-table-column  label="创建时间" show-overflow-tooltip prop="createDates">
            
          </el-table-column>
         <!--  <el-table-column label="我方业务流水号"  show-overflow-tooltip prop="mchOrderId">
          </el-table-column> -->
          <el-table-column prop="orderId" label="订单编号" show-overflow-tooltip width="250px">
          </el-table-column>
          <el-table-column prop="channels" label="订单渠道" show-overflow-tooltip>
          </el-table-column>
          <el-table-column prop="address" label="手机号码" show-overflow-tooltip>
          </el-table-column>
          <el-table-column  label="业务类型" show-overflow-tooltip >话费充值
          </el-table-column>
          <el-table-column prop="amount" label="交易金额" show-overflow-tooltip >
          </el-table-column>
          <el-table-column prop="totalAmounts" label="面值金额" show-overflow-tooltip >
          </el-table-column>
          ...
          <el-table-column prop="processState" label="对账结果" show-overflow-tooltip>
          </el-table-column>
          <el-table-column
...
import { buildRequestURL, formatUnixtimestamp } from '../../../config/util.js';
  export default {
    ...
    methods : {
      ...
      init () {
        ...
        let _this = this;
        this.isDisable=true;
        this.$http.get(buildRequestURL(_this.api.API_GATEWATE+'mango/settle/bill/shop/result/pages',_this.params) ,_this.GLOBAL.topheaders())
          .then(function(res){
            this.isDisable=false;
            if(res){
              ...
              this.tableData.forEach(e=>{
                if(e.createDate){
                  e.createDates = formatUnixtimestamp(e.createDate)
                  
                }
                if(e.tradeAmount){
                  e.amount = e.tradeAmount/100
                }else{
                  e.amount = ''
                }
                if(e.totalAmount){
                ...
                if(e.accountDate){
                  e.accountDates = e.accountDate.substring(0,4) +'-'+ e.accountDate.substring(4,6)+'-'+e.accountDate.substring(6,8)
                }
                if(e.finalDate){
                  e.finalDates = e.finalDate.substring(0,4) +'-'+ e.finalDate.substring(4,6)+'-'+e.finalDate.substring(6,8)

                }
                if(e.channel){
                  for(let item of this.channelList){
                    if(item.tag == e.channel){
                      e.channels = item.name;
                    }
                  }
                }
              })
